...
## 2026-06-15 - [Aggregation via JOIN vs. db.batch]

**Learning:** While `db.batch()` is efficient for multiple independent queries, using a single query with `LEFT JOIN` and `GROUP BY` for simple aggregations (like counts) is even more efficient. It offloads the aggregation to the database engine and eliminates the need for manual result merging in the application layer.
**Action:** Use SQL-level aggregations (`JOIN` + `GROUP BY`) instead of multiple batched queries when fetching related counts or presence indicators.
