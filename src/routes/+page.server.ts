...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Fetch moves, categories, and featured move in a single batch to reduce TTFB on Cloudflare D1.
	...
	const [movesDataRaw, allCategories, featuredMoveBatch] = await db.batch([
		db
			.select({
				id: moves.id,
				name: moves.name,
	...
	const featuredMoveRaw = featuredMoveBatch[0];
