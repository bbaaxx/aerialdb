...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Fetch moves, categories, and featured move in parallel using db.batch()
	// to reduce TTFB by combining multiple queries into a single network round-trip.
	...
	const [movesDataRaw, allCategories, featuredMoveRawList] = await db.batch([
		db
			.select({
				id: moves.id,
				name: moves.name,
	...
	const featuredMoveRaw = featuredMoveRawList[0];
