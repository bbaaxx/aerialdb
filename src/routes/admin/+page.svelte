			...
			<div class="relative">
				...
				{#if searchQuery}
					<button
						type="button"
						onclick={() => (searchQuery = '')}
						class="absolute inset-y-0 right-0 flex items-center pr-3 text-on-surface-variant transition-all hover:text-on-surface focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none active:scale-95"
						...
						<tr class="transition-colors hover:bg-surface-container-high">
							...
							<td class="hidden px-4 py-3 text-center md:table-cell">
								<div class="flex items-center justify-center gap-2">
									{#if move.hasImage}
										<span class="text-lg" role="img" aria-label="Has image" title="Has image">📷</span
										>
									{/if}
									{#if move.hasVideo}
										<span class="text-lg" role="img" aria-label="Has video" title="Has video">🎥</span
										>
									{/if}
