...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Optimized using a single SQL LEFT JOIN and GROUP BY clause
	// to fetch categories and their move counts in one go. This reduces database round-trips
	// and avoids the overhead of manual in-memory merging in the application layer.
	// Using sql<number>`count(${moves.id})` ensures categories with 0 moves are correctly included.
	const categoriesWithCounts = await db
		.select({
			id: categories.id,
			name: categories.name,
			createdAt: categories.createdAt,
			moveCount: sql<number>`count(${moves.id})`
		})
		.from(categories)
	...
	return {
		categories: categoriesWithCounts as CategoryWithMoveCount[]
	};
};
