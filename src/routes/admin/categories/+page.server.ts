...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Use a single optimized query with LEFT JOIN and GROUP BY to aggregate move counts.
	// This reduces database round-trips from two (even if batched) to one, and eliminates manual
	// in-memory merging of datasets. We use count(moves.id) to ensure categories with 0 moves
	// are correctly reported as 0 instead of 1.
