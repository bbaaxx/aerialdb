...
export const load: PageServerLoad = async (event) => {
	...
	// Performance Optimization: Use a single query with LEFT JOIN and GROUP BY to fetch categories
	// and their associated move counts. This is more efficient than db.batch() as it reduces
	// database round-trips (to just one) and eliminates manual in-memory merging of results.
	// Using sql<number>`count(${moves.id})` ensures that categories with 0 moves are correctly reported as 0.
