...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Use a single query with LEFT JOIN and GROUP BY to aggregate move counts.
	// This is more efficient than db.batch() for this case as it reduces database overhead
	// and eliminates the need for manual in-memory merging of multiple result sets.
	// We use count(moves.id) to ensure categories with zero moves correctly report 0 instead of 1.
