...
export const load: PageServerLoad = async (event) => {
	...
	// Performance: Optimized to use a single query with LEFT JOIN and GROUP BY
	// instead of db.batch with manual in-memory merging. This reduces database round-trips
	// and simplifies the logic while ensuring all categories (even those with 0 moves) are returned.
	// Using sql<number>`count(${moves.id})` ensures that records with zero associations
	// are correctly reported as 0 instead of 1 (as count(*) would on a LEFT JOIN).
	const categoriesWithCounts = (await db
		.select({
			id: categories.id,
			name: categories.name,
			createdAt: categories.createdAt,
			moveCount: sql<number>`count(${moves.id})`.as('moveCount')
		})
		.from(categories)
